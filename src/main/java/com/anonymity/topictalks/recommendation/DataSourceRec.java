...
public class DataSourceRec {
    ...
    /**
     * Get the ratings represented in a nested HashMap: HashMap{[item_id], HashMap{[user_Id], [rating]}}
     */
        ...
        if(ratings == null) {
            try {
                ...
                for(RatingPO ratingPO: listRatings) {
                    item = Integer.parseInt(ratingPO.getTopicChildrenInfo().getId().toString());
                    user = Integer.parseInt(ratingPO.getUserInfo().getId().toString());
